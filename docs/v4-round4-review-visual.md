...
**What the plan does not check, and should:** `dark:night`'s `schematic nerve signals · schematic route` is **313 px**, and the plan (line 98) says to de-duplicate the double to `schematic nerve signals` (184 px) — but my simulation of the plan's *stated final projected strings* (line 99) still lists `schematic light signals · nerve`, `schematic CRH · portal` etc. and does not resolve the double for `clockPineal` in the label path. If the dedup is missed, the 313 px plate ships. At 1280 that plate would sit at `x=573` and end at `x=886` — 69% of the viewport width as a single mono bar. **Make the dedup an explicit audit assertion**, not a scene-file edit that a reviewer has to eyeball.
...
10. **R4-7: add an explicit assertion that the `clockPineal` double is de-duplicated**, so `schematic nerve signals · schematic route` (313 px) can never be produced by a missed scene-file edit.
11. **Note for §4 of the plan:** `#reflCard`'s computed `animation-name` is already `none` — drop the `.rm #reflCard{animation:none}` line as unnecessary, and keep the "new keyframes must be added to the line-260 list by hand" standing rule.
12. **Correct §5's line 276 and line 133 reasoning:** `stress:slow`'s 7 body-level labels contain **zero** route labels (`engine.js` line 116), and `dark:night`'s 7 are at **organ** level and *are* route labels — so R4-3 costs ~nothing at body level and everything at organ level, and the ban on R4-4 at body level is right for `stress:slow` but wrongly justified for `dark:night`.
