...
**Learner-facing problem.** The first screen is a body and a column of three labelled rows. Measured from the current build at 1440 × 900: the first interactive element (`#triggers` button) sits at y ≈ 70 px and the body (`#world`, `tabindex="0"`) is the next tab stop — so the *mechanics* are fine. What is absent is the *cue*: the only "start here" signal in the initial viewport is the tip `Start here: pick something that happens to you.` ([app.js](../prototypes/v4/js/app.js) line 91) — and that tip is a ~340 px box whose right edge lands at x ≈ 630, **380 px to the left of the body's centre and directly above the brain cutaway in the same visual column**. It competes with the illustration without pointing at it. Meanwhile the three rows carry a subtitle that describes the *lesson* (`Watch the fast and slow response`) rather than something that *happens to you*, which is what the tip tells the learner to look for. The row and the instruction use different vocabularies.

**Concrete change.**

| File | Function / location | Change |
|---|---|---|
| `js/ui.js` | `HS.TRIGGERS` (lines 74–78) | Add a plain-language alias per trigger: `{id:'stress', title:'Something stressful happens', alias:'an exam, a near miss, a fright', syn:[…], icon:…}`. Same for meal (`you haven't eaten for hours`) and dark (`the evening light fades`). The alias goes into `syn` too, so search picks it up. |
| `js/ui.js` | `HS.renderTriggers` (lines 79–81) | Render the alias as a third line in the row, or as the `small` line where the current subtitle is lesson-flavoured. Keep `aria-pressed` and the `data-trigger` contract untouched. |
| `js/ui.js` | `HS.renderTriggers` | When `HS.E.state !== 'triggered'` (nothing chosen yet), add `class="trig first"` to each row — a single quiet accent (left border, or the existing `.trig.continue` treatment at [app.css](../prototypes/v4/css/app.css) line 328) that makes the column read as *the* way in. It is removed the moment any trigger is pressed. **Not** an animation, **not** a badge, **no count.** |
| `js/app.js` | first paint (line 91) | Move the start tip to be adjacent to the trigger column (`{left:326, top:96}` is already right-aligned to the panel edge; keep it there but shorten the text to match the alias vocabulary: `Start here: pick something that happens to you.` stays — it is already correct). No change needed to the anchor; the change is that the rows now literally list things that happen to you. |

**Effort:** S.

**Acceptance check.** In a 1440 × 900 viewport with tips **off** (so nothing but the layout is doing the work): all three trigger rows are fully visible inside the viewport, each has an alias line, and each row carries `.first`. After clicking any one of them, `.trig.first` is gone from all three.

**Reduced motion / keyboard.** No motion added. The first tab stop after the toolbar is still the first trigger row (`#panel` precedes `#world` in DOM order in [index.html](../prototypes/v4/index.html) lines 50–61); `.first` is a static border, unaffected by `.rm`.

**Risks and guardrails.** No quantity, no gamification: `.first` is an orientation accent on a static list, with no counts, ticks or completion semantics; it disappears on first use and never returns for that session. Salience: the accent lives in the *panel* (already `z-index:15`, `--float` background), not on the body, so it cannot compete with routes or hotspots for attention on the stage itself.
...
|---|---|---|---|
| **R4-1** | **Behavioural, in-page** | In a fresh 1440 × 900 context with `HS.tipsOn=false` and a cleared `localStorage`, take the label boxes of `#triggers button` and of every `#world .org`, plus `#world`'s own box. Compute the **first Tab stop** in the app and assert it is inside `#panel` (i.e. the trigger column precedes the body). Assert the three rows' union box is fully within the viewport and within 40 px of `#world`'s left edge. Then `page.keyboard.press('Enter')` on that first stop and assert `HS.E.state==='triggered'` within 400 ms — i.e. **the first keyboard action available leads to a scene**, which is the scriptable half of "a learner acts within 10 s". Finally assert `.trig.first` count is 0 after the trigger fires. | 6 checks, all green. |
...
|---|---|
| R4-1 | `docs/v4-round2-goal.md` (Phase 16 row) |
...
1. **"The ones still unaddressed by any scripted proxy are tasks 1, 5, 6, 7/8" is half right.** `scripts/v4-comprehension-check.mjs` already exists and already proxies all four — tasks 1, 7 and 8 *pass* (12/14), and tasks 5 and 6 fail. So the actionable gap is **two** tasks, not four. I have kept tasks 1 and 7/8 in the round anyway, but for a different and defensible reason: their existing proxies are **static-attribute probes, not behavioural ones** (T1 checks that a button exists with a subtitle; T7/8 checks that words appear in a sheet). Passing those proves very little about the task. R4-1 and R4-5 convert them into behavioural proxies, which is worth doing — but the headline for this round should be **"two red, two hollow"**, not "four open".
...
3. **R4-4 and R4-6 are the two items I would drop first if the round has to shrink to four.** In that case the surviving four are R4-1, R4-2, R4-3, R4-5 — which are exactly your four tasks, with R4-7 folded into R4-3's commit. I do not recommend shrinking, because R4-4 is the only item that teaches the *carrier grammar* that the direction's §5.3 and the shipped A1 textures already encode, and R4-6 removes a dead end from first paint; but the round is designed so that either cut is clean.
