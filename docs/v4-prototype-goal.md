...
| 7 | Done | Tree follows the WAI-ARIA keyboard model (one tab stop; Up/Down, Home/End, Right expands then enters, Left collapses then goes to parent, type-ahead); hover preview waits 150 ms; mini-map drag pans and click or Enter returns to the whole body; double-click zooms in one step; arrow keys pan when the body has focus; a focused hotspot or label under a floating panel nudges the view into the open area; the side sheet narrows the framing area; Try it? lists its candidates as chips in head-to-pelvis order, synced with the body and marked after Check; "?" opens a keyboard sheet (also in settings); minimum width 1024 px; labels never sit under the open panel. Checked by script: label/label, label/hotspot and label/UI overlaps at 1280, 1440 and 1920 across all five pathways; reduced motion across all five (Next step advances one step, no pulse, no draw-on, no running sign animation, instant reveal) |
| 8 | Done | Artifact republished with all three triggers. 21 evidence screens captured by a Playwright script in `docs/design-review-evidence/v4/` (every scene, close-ups, cell insets, sheets, 1280 and 1920, grayscale). Review note [v4-prototype-review.md](v4-prototype-review.md) lists what was built, the ten places the prototype went beyond the direction (also recorded as §14 of the direction), open owner decisions, and the scientific and anatomy review lists. Fixes found while reviewing evidence: the trigger's second ripple ignored reduced motion and link restores; Try it? and What if? cards covered the organs and badges they refer to, so they now dock right with the camera framing the scene beside them |

**The goal is met:** all three triggers work end to end on one body, and the §10 prototype tasks can be run on this build. What remains is owner decisions, scientific and anatomy review, and the Phase 1 illustration commission, listed in the review note.
