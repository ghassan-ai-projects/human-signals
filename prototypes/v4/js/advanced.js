...
HS.openPassport=function(key,ret){
  ...
  let html=`<button class="x" aria-label="Close">×</button><span class="eyebrow">Advanced · signal passport</span><h3>${pp.name}</h3>${pp.aka?`<p class="sub" style="margin:0">Also called ${pp.aka}</p>`:''}
   <span class="clspill" style="--c:${c.col}">${pp.clsName}</span><div class="ppfig">${figure(pp)}</div><div class="mechanism-block passport-mechanism">${HS.evidenceStatus()}
  ...
  $('#tips').innerHTML='';
  const s=$('#sheet'); s.innerHTML=html; s.setAttribute('aria-label',`${pp.name} passport`); s.classList.remove('closed'); HS.setReaderFocus(true); s.scrollTop=0;
